...
**Abstract** We present AIXBio, a modular agent framework for biological engineering workflows, and demonstrate it by converting human insulin (UniProt P01308) into validated, expression-ready DNA constructs for *E. coli*. Starting from a protein accession, the pipeline retrieves the sequence, optimizes codons, assembles expression cassettes, builds annotated constructs, and runs seven-point validation, producing both insulin chains that pass all checks on the first attempt. AIXBio is built on LangGraph as a two-level state graph with composable nodes, typed state, human-in-the-loop checkpoints, and a full audit trail. The core pipeline is deterministic: an LLM is only invoked as a bounded escalation agent when rule-based remediation fails. The architecture is designed to generalize across host organisms and workflow types. New validation checks, pipeline steps, or expression hosts can be added through a repeatable extension pattern without restructuring the graph. We discuss the properties that make this approach well-suited for safety-critical biological workflows: composability, auditability, bounded AI agency, and testability. AIXBio is a proof of concept intended to explore how agent frameworks can structure and govern multi-step biological workflows while keeping the researcher in control.  |
